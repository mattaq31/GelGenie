                ...
                while len(broken) > 0:
                    ...
                    if len(valid_indices) == 1:
                        ...
                        if np.abs(p1[0] - p2[0]) <= horizontal_threshold:
                            ...
                            current_info['BrokenTrue'].append(list(np.concatenate([wherep1True, wherep2True], dtype=np.int64)))
                            new_centroid = (p1 + p2) / 2
                            ...
                            current_info['SmallTrue'].append(list(np.concatenate([wherep1True, wherep2True], dtype=np.int64)))
...
image_path = os.path.join(folder_path, 'seg_mask_one_hot.png')
binary_image = np.invert(cv2.imread(image_path, cv2.IMREAD_GRAYSCALE))
...
plt.title('Empty Image with broken Blobs')
plt.show(block=False)
...
plt.figure()
plt.imshow(empty_image, cmap='gray')
plt.title('Empty Image with Painted Convex Hulls')
plt.show(block=False)
...
plt.title('Fixed Image with Painted Convex Hulls')
plt.show(block=False)
...
plt.title('Fixed Image with Painted Convex Hulls and New Centroids')
plt.show(block=False)
